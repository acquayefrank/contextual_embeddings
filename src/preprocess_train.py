...
from data import TRAIN_DATA_PATH, WORDS_DATA_PATH, DATA_ROOT as DATA_PATH
...
def hash_data(word, actual_word_features, possible_word_features):
    return hashlib.sha224(f"{word}{actual_word_features}{possible_word_features}".encode('utf-8')).hexdigest()
    ...
    for word in words:
        ...
        if hash_string not in processed_words:
            ...
            if actual_word_features != possible_word_features:
                possible_errors.append([word, actual_word_features, possible_word_features])
                with open(possible_errors_file, 'a+') as file:
                    wr = csv.writer(file, dialect='excel')
...
def process_train_data(threshold=100):
    ...
    df_with_threshold.drop(df_with_threshold[df_with_threshold._sum_of_features <= 0].index, inplace=True)
    del df_with_threshold['_sum_of_features']

    feature_metrics = df_with_threshold.sum(axis = 0, skipna = True)
    feature_metrics[1:].sort_values(ascending=False).to_csv(f'{EVALUATION_PATH}/feature_metrics.csv', header=['count_of_words_having_this_feature'])
...
def enrich_data(clean_train):
    ...
    c_df.to_csv(f'{DATA_PATH}/enriched_data.csv')
    print(c_df.head())
